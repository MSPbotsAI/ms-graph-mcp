...
| `graph_list_owned_groups` | 列出用户拥有的组，每个组附带总owner数——离职场景判断"这个用户是不是唯一owner"（`owner_count==1` 代表移除后立刻变孤儿组）。实现上先用 `/users/{id}` 把 user_id 解析成真实GUID，再用 `/groups/delta?$select=...,owners` 全量分页扫描、逐组核对owners是否包含该用户——app-only下没有任何反查端点：`/users/{id}/ownedObjects` 官方文档写明不支持app-only，`/groups?$filter=owners/any(...)` 会被Graph直接拒绝（`owners`不是Group上的可filter属性），`$expand=owners`挂在列表上有官方文档记录的20条截断且不能翻页的坑，delta是目前唯一同时满足"app-only可用+不会漏数据"的方案，代价是仍要扫描租户内全部group | `Group.Read.All`（还需要 `User.Read.All` 解析user_id，已被现有`User.ReadWrite.All`覆盖） |
...
- **`graph_list_owned_groups` 的实现改过两版**：第一版`/users/{id}/ownedObjects`被官方文档证实不支持app-only；第二版`/groups?$filter=owners/any(...)`在真实客户租户（Precicom sandbox，2qmxyw.onmicrosoft.com）上线后被Graph直接拒绝——`owners`根本不是`/groups`上的可filter属性，这个查询语法本身就不存在，是本仓库的实现bug，不是权限问题。2026-09-02改为第三版：`/groups/delta?$select=...,owners`全量分页扫描+客户端过滤，已过单测（分页+owner_count计算逻辑），但**尚未拿真实租户数据重新验证**，且真实调用延迟会随租户group总数线性增长（无法绕开全量扫描，已查过`$expand`/`$search`等替代方案，均不可行——详见上表)。
